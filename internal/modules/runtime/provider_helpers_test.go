...
func TestProviderRegistryAndManualProvider(t *testing.T) {
	registry := NewProviderRegistry(config.VolcengineConfig{}, config.ComfyUIBridgeConfig{}, config.MinimaxConfig{}, config.KimiCodingConfig{})
	...
	registry = NewProviderRegistry(config.VolcengineConfig{}, config.ComfyUIBridgeConfig{Enabled: true, BaseURL: "http://127.0.0.1:8000"}, config.MinimaxConfig{}, config.KimiCodingConfig{})
